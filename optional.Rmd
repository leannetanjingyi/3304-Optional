...
This project was done using R because I feel that it is the easiest to display the method to replicate my results. 
...
I decided to do a adf.test to see it is stationary as it appears not to be stationary. 

```{r, echo=TRUE}

  #test for stationary
  library(tseries)
  adft <- adf.test(VRoPRU)
  pander(adft)
...
To find the optimal number of lags used in this model, I used the auto.arima function. The reason why I chose to use auto.arima as it can return the best ARIMA model according to the lowest AIC value (see ?auto.arima in R) 
...
#Random walk model
The random walk model is very widely used for non-stationary data, particularly finance and economic data. Random walks also typically have long periods of apparent trends up or down and sudden, unpredictable changes in direction.
